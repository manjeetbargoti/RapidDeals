...
class HomeController extends Controller
{
    ...
    public function singleProperty(Request $request, $rfid=null)
    {
        // dd($request->all());
        if($request->isMethod('post'))
        {
            ...
            $property = Property::where('reference', $rfid)->first();
    ...
    public function singleOffPlan(Request $request, $refernece=null)
    {
        ...
        return view('frontend.property.single_offPlanProperty', compact('properties'));
    }
    ...
    // Property Category Page Function
    public function propertyInCategory(Request $request, $type=null)
    {
        // echo "<pre>";print_r($type);die;

        $type_name = Property::select('t_name')->distinct()->get();
        $properties = Property::where('t_name', $type)->orderBy('created_at', 'desc')->paginate(10);
        $property_count = Property::where('t_name', $type)->count();
        // $properties = json_encode(json_decode($type_name), true);
        // echo "<pre>";print_r($properties);die;
